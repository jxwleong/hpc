...
void histogramEqualisation(PPMImage *image) {
	...
	for (i = 0; i<numData; i+=3) {	// increment by 3 because each RGB pixel is 3 values.
        ...
        // Need to use atomic for this operation
        // To make the operation thread safe
        // Else the data in hist[grey] will be clobbered as
        // all the threads are trying to increment at the same time due to
        // race condition
        #pragma omp atomic
		hist[grey]++;
       //printf("\ni: %i, hist: %ls, grey: %i", i, *hist, grey);
	}
    //printf("\n hist: %ls, grey: %i", *hist, grey);
    ...
    #pragma omp critical
{
    ...
    printf("]\n");
    printf("new_gray_level: [");
    for (i = 0; i < 256; i++) {
        printf("%d, ", new_gray_level[i]);
    }
    printf("]\n");
}
*/
    ...
    // No need parallize this loop because the loop is simple
    // and the iteration is small (not computationally expensive)
    // Using OpenMP for this loop will not bring much benefit for performance
    ...
    // Dont have any shared variables being modified so normal omp parallel
    // for clause does the job
